...
def record_capture(db: Session, payment, amount: int) -> None:
    """
    ...
    Takes an explicit amount rather than always using payment.amount,
    since a capture may be partial — the ledger only ever moves the
    amount actually captured, not the full authorized amount.
    """
